...
function gestionarXml(dadesXml){
 ...
 numeroSecreto2=parseInt(xmlDoc.getElementsByTagName("answer")[5].innerHTML);
 
 var tituloInput3=xmlDoc.getElementsByTagName("title")[9].innerHTML;
 ponerDatosInput3Html(tituloInput3);
 numeroSecreto3=parseInt(xmlDoc.getElementsByTagName("answer")[9].innerHTML);
...
function corregirNumber3(){
  ...
  if (s==numeroSecreto3) {
   ...
   nota +=1;
    ...
    if (s>numeroSecreto3) darRespuestaHtml("P10: Te has pasadoo");
    else darRespuestaHtml("P10: Te has quedado cortoo");
  }
}
  ...
  for (i = 0; i < f.color.length; i++) {  //"color" es el nombre asignado a todos los checkbox
   if (f.color[i].checked) {
	...
	//dividido por el número de respuestas correctas   
     ...
     notaPregunta -=1;//darRespuestaHtml("P7:"+i+"incorrecta");
	 incorrectas +=1;
    }   
   }
  }
  darRespuestaHtml("P3: Respuestas correctas: "+correctas+ ". Respuestas incorrectas: "+ incorrectas+ ". Tu puntacion de la pregunta es:" + notaPregunta);
}
function corregirCheckbox2(){
  ...
  darRespuestaHtml("P7: Respuestas correctas: "+correctas+ ". Respuestas incorrectas: "+ incorrectas+ ". Tu puntacion de la pregunta es:" + notaPregunta);
}
...
function corregirCheckbox3(){
  ...
  darRespuestaHtml("P9: Respuestas correctas: "+correctas+ ". Respuestas incorrectas: "+ incorrectas+ ". Tu puntacion de la pregunta es:" + notaPregunta);
}
...
function corregirRadioButton2(){
  ...
  var f=formElement;
  var escorrecta = [];
  for (i = 0; i < f.pene1.length; i++) {  //"color" es el nombre asignado a todos los checkbox
   if (f.pene1[i].checked) {
    escorrecta[i]=false;     
...
function ponerDatosRadioButton2Html(t,opt){
 ...
 for (i = 0; i < opt.length; i++) { 
    ...
    var label = document.createElement("label");
    label.innerHTML=opt[i];
    label.setAttribute("for", "pene1"+i);
    input.type="radio";
    input.name="pene1";
    input.id="pene1_"+i;;    
...
function presentarNota(){
   darRespuestaHtml("Nota: "+nota+" puntos sobre 8");
}
...
function comprobar(){
   ...
   for (i = 0; i < f.pene.length; i++) {
    ...
    for (i = 0; i < f.pene1.length; i++) {
	if (f.pene1[i].checked) checkedRadioButton2=true;
   }
   ...
   if (f.elements[0].value=="") {
    ...
    alert("Selecciona una opcion del checkbox");
    return false;
	
	} else if (!checkedRadioButton) {    
    document.getElementsByTagName("h3")[3].focus();
    alert("Selecciona una opcion del radioButton");
    return false;
